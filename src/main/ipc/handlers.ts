...
export function registerIpcHandlers() {
   ...
   * 根据 ID 获取服务商
   */
  ipcMain.handle('provider:getById', async (_, id: number): Promise<IpcResponse> => {
    try {
   ...
   * 创建服务商
   */
  ipcMain.handle('provider:create', async (_, data: any): Promise<IpcResponse> => {
    try {
   ...
   * 更新服务商
   */
  ipcMain.handle('provider:update', async (_, id: number, data: any): Promise<IpcResponse> => {
    try {
   ...
   * 删除服务商
   */
  ipcMain.handle('provider:delete', async (_, id: number): Promise<IpcResponse> => {
    try {
      const existingProvider = await providerDal.findById(id);
      if (!existingProvider) {
        return { success: false, error: '服务商不存在' };
   ...
   * 更新服务商状态
   */
  ipcMain.handle('provider:updateStatus', async (_, id: number, status: number): Promise<IpcResponse> => {
    try {
      if (status === undefined) {
   ...
   * 获取指定服务商的模型
   */
  ipcMain.handle('model:getByProvider', async (_, providerId: number): Promise<IpcResponse> => {
    try {
   ...
   * 删除模型
   */
  ipcMain.handle('model:delete', async (_, id: string, provider: number): Promise<IpcResponse> => {
    try {
      if (!id || !provider) {
   ...
   * 批量创建任务
   */
  ipcMain.handle('task:create', async (_, tasks: any[]): Promise<IpcResponse> => {
    try {
   ...
   * 获取任务列表（分页）
   */
  ipcMain.handle('task:getAll', async (_, params: { page: number; pageSize: number }): Promise<IpcResponse> => {
    try {
   ...
   * 更新任务
   */
  ipcMain.handle('task:update', async (_, id: string, data: any): Promise<IpcResponse> => {
    try {
   ...
   * 文件上传（从文件路径复制到任务目录）
   */
  ipcMain.handle('file:upload', async (_, taskId: string, filePath: string): Promise<IpcResponse> => {
    try {
      if (!taskId || !filePath) {
   ...
   * 批量文件上传（支持多文件）
   */
  ipcMain.handle('file:uploadMultiple', async (_, taskId: string, filePaths: string[]): Promise<IpcResponse> => {
    try {
      ...
      for (const filePath of filePaths) {
        const result = await ipcMain.invoke('file:upload', taskId, filePath);
        if (result.success) {
          uploadResults.push(result.data);
        }
      }

      return { success: true, data: { message: '文件上传成功', files: uploadResults } };
    } catch (error: any) {
   ...
   * 图片转 Markdown
   */
  ipcMain.handle('completion:markImagedown', async (_, providerId: number, modelId: string, url: string): Promise<IpcResponse> => {
    try {
   ...
   * 测试模型连接
   */
  ipcMain.handle('completion:testConnection', async (_, providerId: number, modelId: string): Promise<IpcResponse> => {
    try {
