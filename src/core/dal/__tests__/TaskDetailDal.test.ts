...
describe('TaskDetailDal', () => {
  let taskDetailDal: any

  beforeEach(async () => {
    mockReset(prismaMock)
    const module = await import('../TaskDetailDal.js')
    taskDetailDal = module.default
  })
  ...
  describe('findByTaskId', () => {
    it('should return task details sorted by page number', async () => {
      ...
      const result = await taskDetailDal.findByTaskId('task-1')
    ...
    it('should return empty array for task with no details', async () => {
      ...
      const result = await taskDetailDal.findByTaskId('empty-task')
    ...
    it('should order results by page ascending', async () => {
      prismaMock.taskDetail.findMany.mockResolvedValue([])

      await taskDetailDal.findByTaskId('task-1')
  ...
  describe('findByTaskAndPage', () => {
    it('should return task detail for specific task and page', async () => {
      ...
      const result = await taskDetailDal.findByTaskAndPage('task-1', 5)
    ...
    it('should return null when task detail does not exist', async () => {
      ...
      const result = await taskDetailDal.findByTaskAndPage('non-existent', 99)
    ...
    it('should match exact task ID and page number', async () => {
      prismaMock.taskDetail.findFirst.mockResolvedValue(null)

      await taskDetailDal.findByTaskAndPage('task-abc', 10)
  ...
  describe('countByTaskId', () => {
    it('should return correct count of pages', async () => {
      ...
      const result = await taskDetailDal.countByTaskId('task-1')
    ...
    it('should return 0 for task with no details', async () => {
      ...
      const result = await taskDetailDal.countByTaskId('empty-task')
    ...
    it('should count only pages for specified task', async () => {
      prismaMock.taskDetail.count.mockResolvedValue(5)

      await taskDetailDal.countByTaskId('specific-task')
